...
summary: "A short overview of the full RAG build path, from indexing to retrieval, reranking, LLM design, and debugging."
readTime: "7 min read"
tags:
  - "RAG"
  - "Backend"
...
This first log is only a walkthrough. It introduces the full path: indexing, retrieval, reranking, LLM model design, and debugging system design. Each part can be expanded into its own deeper log later.
...
这一篇只是总览，不深入展开。它先把完整路径讲清楚：索引、检索、重排、LLM 模型设计，以及调试系统设计。后面的 log 再逐个模块深入。
